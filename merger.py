...
def main():
    parser = argparse.ArgumentParser(description='小说合并工具 - 将下载的章节合成为TXT或EPUB')
    parser.add_argument('novel_name', help='小说名称（下载目录名）')
    parser.add_argument('-t', '--txt', action='store_true', help='导出TXT格式')
    parser.add_argument('-e', '--epub', action='store_true', help='导出EPUB格式')
    parser.add_argument('-a', '--all', action='store_true', help='导出所有格式（TXT和EPUB）')
    ...
    # 如果没有指定格式，默认导出TXT
    if not (args.txt or args.epub or args.all):
        args.txt = True
...
def interactive_mode():
    ...
    # 获取输入
    novel_name = input("请输入小说名称（下载目录名）: ").strip()
